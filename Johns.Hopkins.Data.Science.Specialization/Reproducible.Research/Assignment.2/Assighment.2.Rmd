...
variable) are most harmful with respect to population health?  
    ...
    + In terms of population health, collectively Tornados, Cyclone, Funnel clouds and heat are the most dangerous events.  Tornados, Cyclone, Funnel clouds cause the most injuries and deaths in total and 2nd most per event, while conversely heat causes the second most total deaths while being the deadliest per event. 
...
#standardize names
        ...
        transform(EVTYPE = gsub(".*cold.*", "COLD", EVTYPE, 
                                ignore.case = TRUE)) %>%
        transform(EVTYPE = gsub(".*wild.*|.*fire.*", "FIRE", EVTYPE, 
                                ignore.case = TRUE)) %>%
        transform(EVTYPE = gsub(".*mud.*|.*land.*", "MUD SLIDE\nLANDSLIDE", 
                                EVTYPE, 
                                ignore.case = TRUE)) %>%
        transform(EVTYPE = gsub(".*heat.*|.*warm.*|.*hot.*", "HEAT", EVTYPE, 
                                ignore.case = TRUE)) %>%
