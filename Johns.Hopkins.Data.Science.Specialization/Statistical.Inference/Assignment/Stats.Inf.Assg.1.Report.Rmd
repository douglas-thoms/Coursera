...
```
The exponential distribution was simulated using the R function rexp(n, lambda). The rate parameter ($\lambda$) is `r lambda` for all of the simulations.    
...
The figure below shows the exponential simulations and the population mean/standard deviation `r pop.mean`. The population mean and standard deviation are both defined as $1/\lambda$.
...
The figure below shows a sample of 1000 exponential distribution means. The theoretical mean `r pop.mean` (which equals the population mean) and sample mean `r round(mean(expo_mean),digits = 2)` are very close as predicted by Central Limit Theorem.
```{r echo}
...
```  
The population variance for an exponential distribution is $1/\lambda^2$.  Therefore, the theoretical variance is calculated using the normal distribution variance formula $Var = \sigma^2/n$ which equals $Var = 1/\lambda^2/n$.  The sample variance `r round(var(expo_mean), digits = 3)` and theoretical variance `r pop.variance/n` are very close, which is also predicted by the Central Limit Theorem.
